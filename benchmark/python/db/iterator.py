...
from db.common import _benchmark_db_prepare_scheme, _benchmark_db_insert,
_benchmark_db_get_identifier, N, N_GET_DIVIDER, CLASS_MODULUS, CLASS_LIMIT
...
def _benchmark_db_get_range(run, namespace, use_index_all, use_index_single):
    ...
    for i in range(N_GET_DIVIDER):
        ...
        range_end_ptr[0] = ((i + 1) * (CLASS_MODULUS / N_GET_DIVIDER)) -
        (CLASS_LIMIT + 1)
        selector = lib.j_db_selector_new(b_scheme, lib.J_DB_SELECTOR_MODE_AND,
                                         b_s_error_ptr)
